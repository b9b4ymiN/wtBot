...
import { BNDivCeil, Base, InstructionType, } from '../base';
import { getATAAddress } from '../base/pda';
import { Clmm } from '../clmm';
import { ASSOCIATED_TOKEN_PROGRAM_ID, AccountMeta, AccountMetaReadonly, Logger, RENT_PROGRAM_ID, SYSTEM_PROGRAM_ID, TOKEN_PROGRAM_ID, findProgramAddress, getMultipleAccountsInfo, parseSimulateLogToJson, parseSimulateValue, simulateMultipleInstruction, splitTxAndSigners, } from '../common';
import { Currency, CurrencyAmount, ONE, Percent, Price, Token, TokenAmount, ZERO, divCeil, parseBigNumberish, } from '../entity';
import { Farm } from '../farm';
import { struct, u64, u8 } from '../marshmallow';
import { MARKET_STATE_LAYOUT_V3, Market } from '../serum';
import { LIQUIDITY_VERSION_TO_STATE_LAYOUT } from './layout';
import { ModelDataPubkey, formatLayout, getDxByDyBaseIn, getDyByDxBaseIn, getStablePrice, } from './stable';
...
export class Liquidity extends Base {
    ...
    static async makeCreatePoolV4InstructionV2Simple({ connection, programId, marketInfo, baseMintInfo, quoteMintInfo, baseAmount, quoteAmount, startTime, ownerInfo, associatedOnly = false, computeBudgetConfig, checkCreateATAOwner = false, makeTxVersion, lookupTableCache, feeDestinationId, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeRemoveAllLpAndCreateClmmPosition({ connection, poolKeys, removeLpAmount, userKeys, clmmPoolKeys, createPositionInfo, farmInfo, computeBudgetConfig, checkCreateATAOwner = false, getEphemeralSigners, makeTxVersion, lookupTableCache, }) {
        ...
        if (farmInfo !== undefined) {
            ...
            for (const item of farmInfo.poolKeys.rewardInfos) {
                ...
                rewardTokenAccounts.push(mintToAccount[item.rewardMint.toString()] ??
    ...
    static computeAnotherAmount({ poolKeys, poolInfo, amount, anotherCurrency, slippage, }) {
        ...
        const liquidity = divCeil(amount.raw.mul(poolInfo.lpSupply), input === 'base' ? poolInfo.baseReserve : poolInfo.quoteReserve);
