...
export class TradeV2 extends Base {
            ...
            for (const iOutPool of info.out) {
                try {
                    ...
                    outRoute.push({
                        ...
                        executionPrice: new Price(maxFirstIn.data.amountIn.amount.token, maxFirstIn.data.amountIn.amount.raw, outC.amountOut.amount.token, outC.amountOut.amount.raw.sub(outC.amountOut.fee?.raw ?? ZERO)),
                        ...
                        minMiddleAmountFee: outC.amountOut.fee?.raw
                            ? new TokenAmount(maxFirstIn.data.amountOut.amount.token, (maxFirstIn.data.amountOut.fee?.raw ?? ZERO).add(outC.amountOut.fee?.raw ?? ZERO))
                            : undefined,
    ...
    static computeAmountOut1({ itemPool, tickCache, simulateCache, chainTime, epochInfo, mintInfos, slippage, outputToken, amountIn, }) {
        if (itemPool.version === 6) {
            const { allTrade, realAmountIn, amountOut, minAmountOut, expirationTime, currentPrice, executionPrice, priceImpact, fee, remainingAccounts, } = Clmm.computeAmountOutFormat({
                poolInfo: itemPool,
