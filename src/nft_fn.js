...
const InferTradeDirection = async (
    wallet,
    logMessages,
    sol_data,
    postTokenBalances) => {
    ...
    const isBuyInstruction = Boolean(
        logMessages.find(
            (message) =>
                ...
                message.includes("Instruction: Buy") ||
    ...
    if (isBuyInstruction) {
        ...
        if (postTokenBalances != undefined && postTokenBalances.length != 0)
            return postTokenBalances[0].owner === wallet ? "BUY 💸" : "SELL 💰";
        else {
            console.log('sol data:', sol_data, Math.sign(sol_data.solChange))
            if (sol_data != null)
                return Math.sign(sol_data.solChange == -1) ? "BUY 💸" : "SELL 💰";
            else
                return "UNKNOW";
        }
    }
...
exports.InferTradeDirection = InferTradeDirection;
exports.InferMarketPlace = InferMarketPlace;
