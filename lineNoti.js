...
const sendData = async (name, data_export) => {
  try {
    ...
    let priceStr = await calPrice(
      data_export.tokenIn_info ? data_export.tokenIn_info.symbol : "SOL",
      data_export.qtyIn,
      data_export.tokenOut_info ? data_export.tokenOut_info.symbol : "SOL",
      data_export.qtyOut
    );
  ...
  try {
    ...
    if (data_export.nftMeta != null) {
      ...
      let message =
        ...
        "\n" +
        "Price : " +
        data_export.nftMeta.price.toFixed(3) +
        "◎\n" +
        "Price : " +
        priceUSD.toFixed(3) +
        " USD\n" +
