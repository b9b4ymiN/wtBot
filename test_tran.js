...
const txn = "2obqNMh2jqA4MbhTCRGfsB1M54BSoGzTVANk8bb2Yv232SxFfZqwkT8qXdiR21UM9p34FrVRa2AnfiBogkxTGvzC"; // Sell token
const txn_buy = "4AQiCWZ3WpzFw2zDC6qJcrPQddbWJYWr97wh1vdtGULs7YH2wT1jZTb7jnjYUsZZJ9fMEjNeT6xoC66uCp2ky1D1";
const tx_buy_NFT = "Sx3qPyqpzoX95g7riVzmcPFrnW6pfpYwKGfR4pDHs1hCooSvYTSxBa2J3yHfESp89GL46A7NJBTtZigW9zm2RtF";

const txJup = "5dN6XidErTZ4kUS17WicXBDhKTkUMSCam31UPHRZEQob2qv9mFtxf2kqgZ3DsAat1Fvhnzki4gTb4vvq5rue76wH";
const tx_2way = "2witM53CsyubKBLrBhK4gAxPsuXY1n2KCcqo3WwgVkYjykaYG6ALBDH42V4XyBQEwVq9z4H5jLRCLAepYKu65m2o" // Convert without SOL
...
const getTransaction = async (txn) => {
    ...
    transactionDetail.meta.err ? console.log(`Status: Failed`) : console.log(`Status: Success`)
    ...
    preTokenBalances.forEach(async (account, i) => {
        let tokenChange = (postTokenBalances[i].uiTokenAmount.uiAmount - account.uiTokenAmount.uiAmount);
        console.log('t : ', postTokenBalances[i].uiTokenAmount.uiAmount )
        console.log('t : ', account.uiTokenAmount.uiAmount )
        let data_token = null;// await getMetadata(account.mint);
        ///console.log(data_token);

        console.log(`   ${i.toString().padEnd(8)} ` +
            `${account.owner.padEnd(45)} ` +
            `${data_token != null ? data_token.name.padEnd(45) : account.mint.padEnd(45)}  ` +
    ...
    transactionDetail.transaction.message.accountKeys.filter(x => x.pubkey.toString() == searchAddress).forEach((account, i) => {
